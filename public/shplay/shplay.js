...
(function (global) {
  ...
  // the same state machine real q5play uses (public/q5play/q5play.js
  // ~6672-6730 InputDevice, ~8118-8122 the per-frame sweep):
  ...
  // the DOM reports ('ArrowRight', ' '), what q5play's docs print
  ...
  // so 166 call sites across 31 lessons — 5-1-20-q5play-move-keys among them —
  ...
  // `kb.pressing('left')` answers for both — q5play's _simpleKeyControls
  // (q5play.js ~7040), which is why its docs can say "arrows or WASD".
  const KEY_DIRS_ = {
  ...
  // spaces/underscores/hyphens, matching q5play's _ac().
  function _key(name) {
  ...
  const kb = {
    ...
    // Real q5play's kb.pressed() is (confusingly, but verified against
    // public/q5play/q5play.js ~6698) an ALIAS of released() — it fires the
    ...
    // Real q5play's releases() is a literal alias of released() (verified
    // against public/q5play/q5play.js:6727-6729: `releases(inp) { return
  ...
  const mouse = {
    ...
    get canvasPos() { return { x: MOUSE.x - camera.x, y: MOUSE.y - camera.y }; },
    ...
    // Same alias relationship as kb.releases() above (q5play.js:6727-6729).
    ...
    // start(). q5play exposes the same name (mouse.isOnCanvas).
    get isOnCanvas() { return MOUSE._onCanvas; },
    // CSS cursor on the canvas element itself — q5play's mouse.cursor
    // (q5play.js:6808-6816) reads/writes $.canvas.style.cursor directly.
    ...
    // sweep (matches q5play's postDraw: `m.scrollDelta.x = 0` etc,
    // q5play.js:8094-8095) — read it once per draw() call, not per event.
    get scrollDelta() { return MOUSE.scrollDelta; },
    // drag/dragging/drags/dragged: q5play's separate per-button counter for
    // "moved while this button was held", NOT the same counter as
    // pressing/presses/released above. Ported from q5play.js:6827-6839 +
  ...
  // left/center/right (default 'left', matching q5play's Mouse._default,
  // q5play.js:6735) -> the MOUSE.* drag-counter slot each name backs.
  ...
  // `kb.pressing('space')`.
  //
  // q5play exposes these as the RAW signed counter, which is truthy on the
  ...
  // space, q5play's idiom for HUDs (q5play.js:8088 turns it off before the
  ...
  function _applyCamera() {
    if (camera._active) CTX_.translate(-camera.x, -camera.y);
  }

  // Named collider-type constants, matching q5play's globals. Their VALUES
  ...
  class Sprite {
    constructor(x, y, w, h, bodyType) {
      ...
      // real p5play Sprite surface (D2: add the knob, keep shPlay's current
      ...
      this._bounciness = 0;
      this._friction = 0;
      // D2/D1: expose density, defaulted to shPlay's current hardcoded 1 —
    ...
    // q5play.js's own get/set pairs (drag ~1835, rotationDrag ~1988,
    ...
    // rotationSpeed is q5play's OWN name for this file's angularVelocity
    // (deg/frame) — a literal alias, both stay live (q5play.js:2014-2027).
    ...
    // every sprite equally regardless of size (q5play.js:2656-2664).
    ...
    // tunnel through a thin wall between physics steps (q5play.js:1873-1879).
    ...
    // re-derive mass rather than preserve it (q5play.js:1912-1920 simplified;
    ...
    // .collider (2.3.x); same underlying bodyType, just q5play's spelling.
    ...
    // but keeps drawing — planck's setActive (q5play.js:1622-1630).
    ...
    // q5play member; direction is degrees, same convention as .direction.
    ...
    // isMoving: q5play's own definition, exactly (q5play.js:1869-1871) —
    ...
    // getter/setter are asymmetric in q5play (q5play.js:1862-1867) — the
    // getter returns a compass STRING (default 'right', q5play.js:985) while
    // the setter just forwards to `.direction` (q5play.js:1817-1833), which
    // only updates that string when it's GIVEN a string, so a later numeric
    // direction write leaves the string stale. `bearing` (q5play.js:1760) is
    ...
    // rotationToFace/angleToFace: port of q5play.js:2718-2734. rotationToFace
    ...
    rotationToFace(x, y, facing) {
      ...
      // q5play's own "too close to rotate toward" guard (q5play.js:2724-2727).
    ...
    // q5play gives (q5play.js:1940-1952, itself just get/set aliases over
    ...
    // numbers or anything with .x/.y (a sprite, mouse, {x,y}) — q5play does,
    ...
    _target(x, y, third) {
      if (x !== null && typeof x === 'object') {
        ...
        // the sprite into the top-left corner. q5play guards this with
        // mouse.isActive (q5play.js:2674); no target is better than a wrong one.
    ...
    // which is exactly why no test caught it. Matches q5play's setter
    // (q5play.js:2049), which takes a number or an {x,y}.
    ...
    // q5play clamps to 0.01 for this reason (q5play.js:2055). At 0.01 a 40px
    ...
    set scale(v) {
      ...
      if (typeof v === 'number') { this._scale.x = this._scale.y = v || 0.01; }
        ...
        // `[2, 3]` as well as `{x: 2, y: 3}` — q5play's typings advertise the
        // array form (q5play.d.ts, `set scale(val: number | [] | {x,y})`) and
    ...
    // The opposite of moveTowards. q5play DECLARES this (docs/q5play.d.ts:910)
    // but ships no implementation for it — verified by grepping q5play.js,
    ...
    // the distance. Direct port of q5play.js:2667.
    attractTo(x, y, force) {
    ...
    // round over several frames. q5play sets .rotationSpeed (q5play.js:2767);
    // shPlay's equivalent unit is angularVelocity, also degrees per frame.
    rotateTowards(angle, tracking) {
      ...
      //
      // q5play normalises only its object/coordinate form (via angleToFace +
      // minAngleDist); the plain-number form takes the delta raw
      // (q5play.js:2782, `angle -= this.rotation`), so rotateTowards(350) from
      // rotation 0 spins 350 degrees the long way round instead of 10 the
      // short way. We diverge deliberately — see DECISIONS.md D17. shPlay's
      ...
      // they turn is a worse trap than disagreeing with q5play.
    ...
    // capability — they exist because a student arriving from p5/q5play
    ...
    // `sprite.position` is real q5play's own name for `sprite.pos` (verified
    // against public/q5play/q5play.js ~2187) — plain alias, same live vector.
    ...
    set angularVelocity(degPerFrame) {
      ...
      // rotate anyway with nothing to explain why. q5play has that hole too;
    ...
    // `_overlap` counter below — real q5play's overlaps() IS edge-triggered,
    ...
    // q5play's own Sprite (no default assignment anywhere in its
    // constructor; only its Group.Visuals subclass defaults to Infinity,
    // q5play.js:580) — so `undefined - 1` is NaN, never <= 0, and an
    ...
    // colour/fill: both directions, plain aliases of .color — q5play calls
    // it colour in its own props list (q5play.js:3093) and fill is the
    // q5.js-global name (this file's own `fill(c)` sets DEFAULT text/shape
    // color; sprite.fill is unrelated to it, matching q5play's per-sprite
    ...
    // textFill is real q5play's actual name for what this file calls
    // textColor (q5play.js:1719-1725) — aliased both directions onto the
    // SAME storage, so old code using textColor and new code using textFill
    // agree. Default stays shPlay's 'white', not q5play's black (D1: keep
    // shPlay's effective defaults, adopt the surface).
    ...
    // below only when a stroke color is set (q5play.js:1727-1741).
  ...
  class Group extends Array {
    ...
    push(...sprites) {
      for (const s of sprites) {
        if (this.includes(s)) continue; // already a member — a no-op, like real q5play
    ...
    // same thing under two names (real q5play documents both spellings) —
    ...
    // amount: q5play's target-count property (q5play.js:4080-4091) — get
    ...
    // skipped — this file already has the factory real q5play's version
    ...
    // q5play (q5play.js:4383-4439) it does.
    ...
    cull(top = 0, bottom, left, right, cb) {
      ...
      const minX = camera.x - left, minY = camera.y - top;
      const maxX = camera.x + W_ + right, maxY = camera.y + H_ + bottom;
      let culled = 0;
  ...
  // Real signature (q5play.js:4984): getSpritesAt(x, y, radius, group,
  ...
  // parity and silently ignored — it queries q5play's own camera-active
  ...
  // matters when a radius is given). Not a q5play member — this file's own
  ...
  // explodeAt(x, y, radius, magnitude, falloff) — q5play's argument order
  // (q5play.js:4971), including its leading-object form so
  // `world.explodeAt(sprite, 120, 40)` works.
  //
  // q5play delegates to Box2D v3's b2World_Explode. planck is a Box2D 2.x
  ...
  // exactly once per frame (shPlay has no begin-contact/end-contact
  ...
  function render() {
    ...
    //
    // q5play's answer to a HUD is camera.off() around drawing PRIMITIVES,
    // which works here too — but shPlay renders sprites for you, so a HUD
    // built out of sprites had no way to stay put and scrolled off with the
    // level. `sprite.screenSpace = true` is ours, not q5play's. Screen-space
  ...
  // sketch produces byte-identical draw ops" (shplay-harness.mjs header).
  ...
  // given — shplay's loop is a fixed rAF cadence with no retargeting
  ...
  // physics-backed proxy with a plain vector, exactly like real q5play).
  class Vector {
  ...
  // would mean changing shplay's boot sequence (`start()`), which is out of
  ...
  function start() {
    ...
    window.addEventListener('keyup', (e) => {
      ...
      // still down. q5play releases it unconditionally (q5play.js ~7009): hold
    ...
    CANVAS_.addEventListener('mousemove', (e) => {
      ...
      // q5play's onpointermove (q5play.js:6997-7001). Consumed and cleared
    ...
    // seen at all — q5play.js:6744-6745 keeps the same two flags separate).
    ...
    // matching q5play's postDraw reset (q5play.js:8094-8095).
    ...
    window.addEventListener('mouseup', (e) => {
      ...
      // drag release: a flat -1 (q5play.js:7019), not the holdThreshold
    ...
    function loop(now) {
      ...
      // physics", matching where q5play's own allSprites.update() runs
      // relative to physicsUpdate (q5playPreDraw, ~8049-8057) — a sprite's
      ...
      // Matches real q5play's postDraw sweep (q5play.js ~8118-8122): a
      ...
      // like every other counter here — q5play.js:8097-8107.
