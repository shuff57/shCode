// shplay-checks-parity.mjs — parity checks against REAL p5play v3 semantics.
//
// shplay-checks.mjs proves the engine runs; this file proves it matches the
...
//
// Same contract as shplay-checks.mjs: `run()` returns `true` to pass, or a
...
export const PARITY_CHECKS = [
  // ---- collision ----------------------------------------------------------
  {
    ...
    run({ runSketch }) {
      ...
      // (shplay.js ~302-315) — here `other` is a lone Sprite, so cb is never
  ...
  },
  {
    ...
    area: 'collision',
    run({ runSketch }) {
      // Sprite._bounds() (shplay.js ~292-296) always returns a plain AABB,
      // even for circle sprites — _overlapsOne() then does rectangle math on
      // that AABB. world.getSpriteAt() (shplay.js ~472-485) does true
  ...
  // ---- input ----------------------------------------------------------------
  {
    ...
    // holds/holding/held/released. shPlay's own `pressed`/`released` are
  ...
  // ---- joints ---------------------------------------------------------------
  {
    ...
    area: 'joints',
    run({ runSketch }) {
      // shplay.js _anchor(): `(opt.anchor && opt.anchor.x) || a.x` — `||`
  ...
  },
  {
    ...
    area: 'joints',
    run({ runSketch }) {
      // shplay.js DistanceJoint: `(opt.length) ? opt.length/PXM : sep` — a
  ...
  },
  {
    ...
    area: 'joints',
    run({ runSketch }) {
      // shplay.js GrabberJoint: `(opt.maxForce) || 100` — same falsy-0 bug.
      const r = runSketch(`
        function setup(){ new Canvas(400,400); world.gravity.y = 0;
          a = new Sprite(200,200,20,20,'static');
          b = new Sprite(220,200,20,20);
  ...
  // ---- sprite-props -----------------------------------------------------
  {
    ...
    // DECISION D1: shPlay keeps its own defaults on purpose — adopting
    ...
    run({ runSketch }) {
      ...
      if (densT !== 'number') return `s.density is ${densT} (${densV}), expected the number 5 — shPlay has no density property at all`;
  ...
  // ---- physics --------------------------------------------------------------
  {
    ...
    // to {x:0, y:0} (no gravity until a sketch sets it). shPlay's `_world()`
    ...
    // (see report) — 270 working lessons may depend on shPlay's default.
    name: 'world.gravity defaults to (0, 0), matching p5play (no gravity until a sketch sets it)',
    // DECISION D1: shPlay turns gravity ON by default so a student's first
  ...
  {
    name: 'overlaps() is edge-triggered like q5play (fires only on the first frame)',
    area: 'collision',
    // DECISION D11: shPlay's overlaps() is LEVEL-triggered on purpose. The
    ...
    run({ runSketch }) {
      ...
      return `overlaps() reported true on ${hits} frames of one pass-through; q5play fires once. shPlay is level-triggered by curriculum design (D11) -- collides()/overlapping()/overlapped() carry the q5play-faithful edge and count semantics`;
    },
  },
];
