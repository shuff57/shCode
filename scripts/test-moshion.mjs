#!/usr/bin/env node
// test-moshion.mjs — the moSHion acceptance gate.
//
// Two halves, matching the two halves of the bar:
//   CORPUS  every runnable piece of moSHion code in the course must execute
...
// public/moshion/moshion.js, never by loosening a check here.
//
//   node scripts/test-moshion.mjs            # everything
//   node scripts/test-moshion.mjs --corpus   # course corpus only
//   node scripts/test-moshion.mjs --semantic # behaviour checks only
//   node scripts/test-moshion.mjs --json     # machine-readable, for critics

import { readFileSync, readdirSync, existsSync, statSync } from 'node:fs';
import { join, dirname, relative } from 'node:path';
import { Script } from 'node:vm';
import { fileURLToPath } from 'node:url';
import { runSketch, createSandbox } from './moshion-harness.mjs';
import { SEMANTIC_CHECKS } from './moshion-checks.mjs';
...
const PARITY_CHECKS = await optionalChecks('./moshion-checks-parity.mjs', 'PARITY_CHECKS');
...
const SURFACE_CHECKS = await optionalChecks('./moshion-checks-surface.mjs', 'SURFACE_CHECKS');
...
// A snippet only counts as moSHion code if it actually touches the engine.
// Plain-JS lessons run in a different runner and are not this gate's business.
const MOSHION_MARKER = /\b(new\s+Canvas|new\s+Sprite|new\s+Group|allSprites|world\s*\.\s*gravity|kb\s*\.|mouse\s*\.\s*(pressing|presses|released)|new\s+\w*Joint)\b/;
...
// Only `plain` opts out of moSHion: LiveCodeBlock renders the moSHion iframe
  ...
  for (const id of lessonDirs()) {
    ...
    if (existsSync(mdPath)) {
      for (const b of liveBlocks(readFileSync(mdPath, 'utf8'))) {
        if (!MOSHION_MARKER.test(b.code)) continue;
        items.push({
          kind: 'live-block',
          lesson: id,
...
function firstUserFrame(err) {
  if (!err?.stack) return '';
  const line = String(err.stack).split('\n').find((l) => /\.js:\d+/.test(l) && !/moshion\.js|planck/.test(l));
...
async function main() {
  ...
  if (report.corpus.length) {
    console.log(`\n${C.b}CORPUS${C.x} — every runnable moSHion sketch in the course`);
...
${C.b}DELTAS vs p5play${C.x} ${C.d}(deliberate — see _workspace/gauntlet/DECISIONS.md D1; these never gate)${C.x}`);
  ...
  console.log(`\n${ok ? C.g + 'moSHion gate: PASS' : C.r + 'moSHion gate: FAIL'}${C.x}\n`);
  process.exit(ok ? 0 : 1);
}
