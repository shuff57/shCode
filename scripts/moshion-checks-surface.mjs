...
export const SURFACE_CHECKS = [
  ...
  },
  {
    ...
    run({ runSketch }) {
      ...
      // The camera is CENTRED on camera.x/y, so on a 400x400 canvas moving it
      // to (100,40) puts the viewport's top-left corner at world (-100,-160)
      // and the sprite at (250,180) lands at screen (350,340).
      if (!near(b.scrolled.x, 350) || !near(b.scrolled.y, 340))
        return 'canvasPos after camera moved to (100,40) was ' + JSON.stringify(b.scrolled) + ', expected (350,340)';
    ...
    run({ runSketch }) {
      const r = runSketch(`
        function setup(){ new Canvas(400,400); world.gravity.y = 0;
          ...
          // (the reference implementation - _update() calls _customUpdate only when set).
          // An earlier version of this check asserted the sprite stopped
          // moving, which the reference API never promised.
          quietTicks = 0;
...

  {
    ...
    // with the level because moSHion renders sprites for you and nothing could
    ...
    run({ runSketch }) {
      const r = runSketch(`
        ...
        function draw(){
          ...
          if (frameCount === 3) {
            ...
            // Screen pixel -> world is pixel + (camera - half the canvas),
            // because camera.x/y is the CENTRE of the viewport. Canvas is 400.
            hudHit = world.getSpritesAt(hud.x + camera.x - 200, hud.y + camera.y - 200).includes(hud);
      ...
      // camera.x=300 on a 400-wide canvas leaves the viewport's left edge at
      // world 100, so the translate is -100, not -camera.x.
      const translates = r.box.ops.filter((o) => o.op === 'translate' && o.args[0] === -100);
      ...
      if (b.worldScreen !== 40 - 100)
        return 'a world sprite at x=40 with camera.x=300 reported canvasPos.x ' + b.worldScreen + ', expected -60';
  ...
  },
  {
    ...
    run({ createSandbox }) {
      ...
      // scrollDelta is a VECTOR in the reference API (its sweep zeroes .x and .y
