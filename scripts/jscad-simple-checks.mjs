...
// ===========================================================================
// THE PORTABLE FORM — lib/jscad-portable.mjs, and how it is kept honest.
// ===========================================================================
//
// curriculum-plan.md names https://jscad.app/ as the Q3-Q4 environment ("no
// install required"), and an shCAD program does not run there. Two things stand
// in the way, not one: the twelve shCAD names, and the 124 BARE names
// runner.html's shim installs. EQUIVALENTS' own revolve row is a name of each
// kind in a single expression — revolve(translate([10, 0, 0], rect(4, 10))) —
// so a converter that fixes only the first half still produces a file that dies
// on "translate is not defined".
//
// THE HARD PART IS THAT EQUIVALENTS HOLDS FUNCTIONS AND A CONVERTER NEEDS TEXT.
// It is closed with Function.prototype.toString(). The rows are arrow
// functions, so their source IS the text, and `.mjs` puts no build step between
// the bytes on disk and the bytes node runs:
//
...
//
// Strip the arrow head and the one-letter prefix and the shCAD half is exactly
// what a student writes, while the real half is already MODULE-QUALIFIED —
// which is the portable spelling. That is the piece of luck the whole design
// rests on: the pairs are simultaneously executable (the identity bar) and
// readable (the converter's corpus), so there is no second table of strings
// anywhere and nothing to drift.
//
// Four checks come off each row: the text matches, the converted program RUNS
// in the require-only context and builds identical geometry, the UNCONVERTED
// program fails there (the negative control — the day it passes, either the
// shim leaked into jscad.app or the converter has no reason to exist), and the
// derived require header is right, which the running proves.
//
// And three interlocks, so the converter cannot fall behind the layer:
//   1. one rule per SHCAD_NAMES entry, no missing and no extra
//   2. every rule exercised by at least one corpus row (turn excepted — it has
//      no single-call equivalent, so it is proved against TURN_IN_PLACE and
//      TURN_COMPOSITION instead, the tables that already own it)
//   3. every declared branch actually taken

export const PORTABLE = {
  path: join(REPO, 'lib/jscad-portable.mjs'),
  helper: 'turnInPlace',
  /** turn is the one rule with no EQUIVALENTS row, by design */
  provedElsewhere: ['turn'],
  /** and what its conversion must NOT be, because rotate pivots on the world */
  notAPlainRotate: /^transforms\.rotate\(/,
};

/**
 * The source text of an EQUIVALENTS half. The parameter name is READ off the
 * function rather than assumed to be `w` / `j`, so renaming it in the table
 * cannot silently produce a half-stripped string that then fails to compare.
 */
export function equivalentText(fn) {
  const src = String(fn);
  const arrow = src.indexOf('=>');
  if (arrow < 0) return { error: 'not an arrow function' };
  const param = (src.slice(0, arrow).match(/[A-Za-z_$][\w$]*/) || [])[0];
  if (!param) return { error: 'no parameter to strip' };
  return { text: src.slice(arrow + 2).trim().replace(new RegExp(`\\b${param}\\.`, 'g'), '') };
}

/** Token-for-token comparison, so line wrapping in the table is not a failure. */
export function normaliseCall(text) {
  return String(text).replace(/\s+/g, ' ').replace(/\s*([(),:{}[\]])\s*/g, '$1').trim();
}

/** Wrap a bare expression in the program shape jscad.app actually runs. */
export function programAround(expression) {
  return `function main() {\n  return ${expression}\n}\n`;
}

/** The expression a converted program returns, for the text comparison. */
export function returnedExpression(code) {
  const m = /return ([\s\S]*?)\n\}/.exec(code);
  return m ? m[1] : '';
}

/**
 * Corpus rows EQUIVALENTS cannot supply. Each is here because a branch or a
 * real /sandbox output needs proving, not because more is better.
 *
 *   sit(parts)      the one textually undecidable rule — see the comment on it
 *                   in lib/jscad-portable.mjs. The only row that exercises the
 *                   `identifier` branch.
 *   the mixed line  an shCAD call, a bare shim name and a real namespaced call
 *                   in one expression, which is what a half-converted file
 *                   looks like — and the reason the converter does two jobs.
 *   the sandbox     the three shapes the /sandbox generator emits that had no
 *                   shCAD twin until this round.
 */
export const PORTABLE_FIXTURES = [
  {
    label: 'sit(parts) — grouped decided at run time',
    program: 'function main() {\n'
      + '  const parts = [ball(10), translate([0, 0, 30], box(10, 10, 10))]\n'
      + '  return sit(parts)\n}\n',
    shcad: (w) => w.sit([w.ball(10), w.translate([0, 0, 30], w.box(10, 10, 10))]),
    notes: [/Array\.isArray/],
  },
  {
    label: 'an shCAD name, a bare shim name and a real call in one expression',
    program: 'function main() {\n'
      + '  return subtract(extrude(6, rect(40, 20)), cylinder({ radius: 4, height: 40 }))\n}\n',
    shcad: (w) => w.subtract(w.extrude(6, w.rect(40, 20)), w.cylinder({ radius: 4, height: 40 })),
  },
  {
    label: 'the three shapes /sandbox generates, in one model',
    program: 'function main() {\n'
      + '  const corners = [[0, 0], [20, 0], [10, 15]]\n'
      + '  return sit([cone(10, 20), translate([40, 0, 0], ring(14, 4)), '
      + 'translate([-40, 0, 0], extrude(6, poly(corners)))])\n}\n',
    shcad: (w) => w.sit([
      w.cone(10, 20),
      w.translate([40, 0, 0], w.ring(14, 4)),
      w.translate([-40, 0, 0], w.extrude(6, w.poly([[0, 0], [20, 0], [10, 15]]))),
    ]),
  },
  {
    // A template literal is TWO things, and reading it as one produced the
    // worst output this converter can produce: a file reported CLEAN — no
    // refusal, no note — that then dies on jscad.app with "measureVolume is not
    // defined", on a line the student was told was fine.
    // Both kinds of name live in the hole — `measureVolume` is one of the 124
    // bare names the shim installs and does not exist on jscad.app, and `ball`
    // is shCAD's. Both are converted; the text around them is not. The measured
    // shape is a throwaway on purpose: measuring the RETURNED one caches a
    // field on it and the geometry compare then reports a difference that is
    // the measurement, not the conversion.
    label: "a template literal — its text stays text, its ${ } holes are code",
    program: 'function main() {\n'
      + '  const s = sit(box(40, 20, 10))\n'
      + '  const say = `not a call: box(9, 9, 9) — volume ${measureVolume(ball(2))}`\n'
      + '  console.log(say)\n'
      + '  return s\n}\n',
    shcad: (w) => w.sit(w.box(40, 20, 10)),
    keep: ['not a call: box(9, 9, 9)', 'measurements.measureVolume(primitives.sphere({ radius: 2 }))'],
  },
  {
    // THREE DOTS ARE NOT A DOT. The scanner classifies a name by the character
    // before it, and a spread ends in one: in `union(...sit(parts))` the
    // character before `sit` is the third `.` of `...`. One character of
    // lookbehind read that as member access, left `sit` bare, and reported the
    // file CLEAN — no refusal, no note — and it then died on jscad.app with
    // "sit is not defined". Same shape as the template-literal defect above and
    // the same root: too little context, and then silence about it.
    //
    // Spread is taught material here — reference.md's own boolean section
    // writes booleans.union(...parts) — so this is a program a student writes,
    // not a corner. Both kinds of name go behind the dots: shCAD's own `sit`
    // here, and one of the 124 bare shim names in the row below.
    label: 'a spread argument — `...` is three dots, not a member access',
    program: 'function main() {\n'
      + '  const parts = [ball(10), translate([0, 0, 30], box(10, 10, 10))]\n'
      + '  return union(...sit(parts))\n}\n',
    shcad: (w) => w.union(...w.sit([w.ball(10), w.translate([0, 0, 30], w.box(10, 10, 10))])),
    keep: ['booleans.union(...transforms.align('],
  },
  {
    label: 'a spread of a bare shim name, which does not exist on jscad.app either',
    program: 'function main() {\n'
      + '  return union(...scission(subtract(ball(10), box(30, 30, 6))))\n}\n',
    shcad: (w) => w.union(...w.scission(w.subtract(w.ball(10), w.box(30, 30, 6)))),
    keep: ['booleans.union(...booleans.scission('],
  },
  {
    label: 'a parameter panel survives, and its defaults still reach main()',
    program: 'function getParameterDefinitions() {\n'
      + "  return [{ name: 'w', type: 'number', initial: 40 }]\n}\n"
      + 'function main(params) {\n  return sit(box(params.w, 20, 10))\n}\n',
    shcad: (w) => w.sit(w.box(40, 20, 10)),
    exports: /module\.exports = \{ main, getParameterDefinitions \}/,
  },
];

/**
 * REFUSAL RATHER THAN CLEVERNESS, as fixtures.
 *
 * There is no parser here — no new dependency means no acorn — so the rewrite
 * is a hand-rolled scan that skips strings, comments, member access and
 * property-key positions. Those are the sharp edges. Both halves are asserted:
 * text the scanner must leave untouched has to come through byte-identical, and
 * text it cannot classify has to be REPORTED with a line number rather than
 * guessed at.
 */
export const PORTABLE_LEAVES_ALONE = [
  { what: 'a shCAD name inside a string',
    program: "const label = 'box(1, 2, 3)'\n", keep: "'box(1, 2, 3)'" },
  { what: 'a shCAD name inside a // comment',
    program: '// turn(45, s) used to be here\nconst n = 1\n', keep: '// turn(45, s) used to be here' },
  { what: 'a shCAD name inside a /* */ comment',
    program: '/* disc(6) */\nconst n = 1\n', keep: '/* disc(6) */' },
  { what: 'a property of something else',
    program: 'const n = shapes.disc\n', keep: 'shapes.disc' },
  // The other side of the spread fix: a dot NOT preceded by another dot is
  // still member access, and `?.` has to go on counting as one.
  { what: 'a property reached with ?., which is still member access',
    program: 'const n = model?.turn\n', keep: 'model?.turn' },
  { what: 'a key in an object of your own',
    program: 'const sizes = { tube: 1, box: 2 }\n', keep: '{ tube: 1, box: 2 }' },
  { what: 'real names that are also English words, as keys',
    program: 'const o = { center: 1, line: 2, text: 3 }\n', keep: '{ center: 1, line: 2, text: 3 }' },
  // The other half of the template-literal fixture above: the CHUNKS are text.
  { what: "a shCAD name in a template literal's text",
    program: 'const label = `box(1, 2, 3)`\n', keep: '`box(1, 2, 3)`' },
  // A regex is not division, and the scan used to walk into one and rewrite its
  // insides — a WRONG rewrite, which is worse than a refusal, and silent.
  { what: 'a shCAD name inside a regex literal',
    program: 'const re = /box(1, 2, 3)/\n', keep: '/box(1, 2, 3)/' },
  { what: 'division, which is what a slash after a value really is',
    program: 'const n = (40 + 20) / 2 / 3\n', keep: '(40 + 20) / 2 / 3' },
];

export const PORTABLE_REFUSES = [
  {
    what: 'a name the file declares for itself',
    program: 'function main() {\n  const box = 3\n  return ball(box)\n}\n',
    says: [/declares `box` itself/],
    // The right answer is BOTH: ball() is converted, and `box` — which is the
    // student's own number here, not shCAD's name — is left exactly as written.
    keep: 'primitives.sphere({ radius: box })',
  },
  {
    // The anchor for the spread fix on the refusal side. Read `...box` as
    // member access and there is no refusal at all, because the classifier is
    // never reached — so a green here means the three dots got past it, and
    // `box` was weighed against the file's own declarations like any other name.
    what: 'a spread of a name the file declares for itself',
    program: 'function main() {\n  const box = [ball(1)]\n  return union(...box)\n}\n',
    says: [/declares `box` itself/],
    keep: 'booleans.union(...box)',
  },
  {
    what: 'an options object that is an identifier, not a literal',
    program: 'function main(params) {\n  const extras = {}\n  return box(40, 20, 10, extras)\n}\n',
    says: [/not a \{ \} written out here/, /will not guess/],
  },
  {
    what: 'sit handed something that is neither a list nor a call',
    program: 'function main() {\n  return sit(a ? b : c)\n}\n',
    says: [/cannot be read off the text/],
  },
  {
    what: 'a shorthand property, which could be either thing',
    program: 'function main() {\n  const o = { disc }\n  return o\n}\n',
    says: [/shorthand property/],
  },
  {
    // Not a rewrite decision — the file is already not valid JavaScript — but
    // everything after the stray ` was read as text, and silence about that is
    // the same silence the template-literal fix exists to remove.
    what: 'a template literal that is never closed',
    program: 'function main() {\n  const s = `oops ${box(1, 2, 3)}\n  return ball(1)\n}\n',
    says: [/never closes/, /read as text/],
  },
  {
    what: 'a file that already declares turnInPlace',
    program: 'function turnInPlace(a, b) { return b }\n'
      + 'function main() {\n  return turn(45, box(10, 10, 10))\n}\n',
    says: [/already declares turnInPlace/],
  },
];

/**
 * ONE WITNESS PER MODULE, SO THE REQUIRE HEADER IS PROVED WHERE THE CORPUS
 * CANNOT REACH.
 *
 * The header is derived, not written: the converter scans what it emitted and
 * destructures the modules it finds. Every corpus row proves that derivation
 * for the modules that row happens to touch — and the rows between them touch
 * ten of the fifteen. Measured on this tree: deleting `text` from MODULE_ORDER
 * left the gate at a FULL PASS, because nothing in the corpus or the fixtures
 * calls vectorText or vectorChar. A student's nameplate program would convert
 * to a file whose header never destructures `text`, and die on jscad.app with
 * "text is not defined" — with every check green.
 *
 * Adding a fixture for `text` would close today's hole and leave the same one
 * open for the next module nothing happens to use, which is the fix that does
 * not hold. So the witnesses are DERIVED FROM MODULE_OF: for each module in
 * MODULE_ORDER, the first bare name the map files under it. A module with no
 * member name at all is returned with `name: null` rather than skipped, because
 * a silently-empty row is exactly the shape of the hole this closes.
 *
 * The witness is used with `typeof`, not with a call: the point is that the
 * header pulled the module in and the qualified path resolves there, and
 * `typeof` proves both for a function, an object and a table of colours alike
 * with no argument list to invent.
 */
export function moduleWitnesses(moduleOf, moduleOrder) {
  const first = new Map();
  for (const [name, path] of Object.entries(moduleOf)) {
    const dot = path.indexOf('.');
    if (dot < 0) continue;          // the module's own bare name proves nothing
    const module = path.slice(0, dot);
    if (!first.has(module)) first.set(module, { module, name, path });
  }
  return moduleOrder.map((m) => first.get(m) || { module: m, name: null, path: null });
}

/**
 * THE LAYER AND THE CONVERTER HAVE TO AGREE ABOUT WHAT A PROGRAM MEANS.
 *
 * simple.js's readOptions refuses an unknown key BY NAME — a misspelled option
 * that silently does nothing is the defect the whole layer exists to close.
 * The converter re-implements option handling from text, and until this table
 * existed it validated nothing. Measured, fourteen name/key pairs behaved this
 * way: `revolve(profile, { center: [1, 2, 3] })` throws in shCode with a
 * message naming the key, and converted to
 * `extrudeRotate({ center: [1, 2, 3] }, profile)` — which the library ignores.
 * `disc(20, { roundRadius: 1 })`, `ball`, `cone` and `extrude` the same. The
 * student is then told their file is CLEAN and gets a different model on
 * jscad.app from the one shCode refused to build at all.
 *
 * The property asserted is ONE-DIRECTIONAL and that is deliberate: the
 * converter must never ACCEPT a call the layer refuses. The other direction is
 * safe by construction — a converter refusal is reported with a line number and
 * the text is left alone, which is this file's whole stance — so requiring
 * exact parity would be requiring the converter to guess.
 *
 * The calls are BUILT from SHCAD_NAMES rather than listed, so a thirteenth name
 * inherits the matrix the day its row lands. The vocabulary below is keyed by
 * PARAMETER name, not by function name, which is what makes that inheritance
 * real; a name that introduces a parameter with no sample fails loudly instead
 * of being skipped.
 *
 * The values are chosen so that every key which is ALLOWED somewhere is also
 * legal there — 20 across the board leaves room for roundRadius 1 — so an
 * accepted case can be held to the geometry bar rather than merely to "it did
 * not throw". `thickness` is the control: no name has it, so it must be refused
 * twelve times out of twelve, and a matrix where it is not is a matrix that has
 * stopped asking anything.
 */
export const PORTABLE_SAMPLE_ARGUMENT = {
  width: '20', depth: '20', height: '20', radius: '20',
  ringRadius: '14', tubeRadius: '4', degrees: '45',
  points: '[[0, 0], [20, 0], [10, 15]]',
  // Offset off the axis, because revolve spins about it and a profile that
  // straddles it is a different refusal than the one being measured.
  shape: 'translate([20, 0, 0], rect(20, 10))',
};

export const PORTABLE_OPTION_PROBES = {
  segments: '32',
  roundRadius: '1',
  center: '[0, 0, 0]',
  thickness: '2',
...
/**
 * `name(a, b)` for one SHCAD_NAMES row, with an optional extra argument. The
 * variadic tail is dropped: `...moreShapes` is a repeat of a slot that is
 * already filled, and repeating it would push the trailing { } out of the
 * position being measured.
 */
export function sampleCall(spec, extra) {
  const args = [];
  for (const param of spec.positional) {
    if (param.startsWith('...')) continue;
    const sample = PORTABLE_SAMPLE_ARGUMENT[param];
    if (sample === undefined) {
      return { error: `no sample argument for the parameter \`${param}\` — add one to `
        + 'PORTABLE_SAMPLE_ARGUMENT, so the option matrix keeps covering every name' };
    }
    args.push(sample);
  }
  if (extra !== undefined) args.push(extra);
  return { text: `${spec.name}(${args.join(', ')})` };
}

/**
 * A CORPUS ROW IS A POINT SAMPLE, AND A POINT SAMPLE CANNOT PROVE A MAPPING.
 *
 * The four checks each EQUIVALENTS row runs compare the rule's emitted text to
 * that row's own real half, for that row's own literals. Nothing in them forces
 * a rule to be a FUNCTION of its arguments — so any positional slot the corpus
 * happens to sample at exactly one value can be replaced by that value, spelled
 * out, and every check stays green.
 *
 * That is not hypothetical. Measured on this tree: hard-coding cone's
 * `startRadius` to `[10, 10]`, cone's height to `20`, tube's radius to `5`,
 * tube's height to `20`, poly's points to the corpus's own three corners,
 * turn's angle to `90`, or revolve's whole profile to the corpus's own
 * translate(...) — each on its own, and all of them at once — left the gate at
 * a full PASS. Convert a student's program against that build and their
 * disc(7) profile is silently GONE, replaced by the corpus's rectangle.
 *
 * `ring`, `disc` and `ball` survived it only by accident: each happens to have
 * two rows with different numbers, so a hard-coded slot disagrees with one of
 * them. Adding a second row for the other five would close today's holes and
 * leave the next name added with the same one open, which is a fix that does
 * not hold.
 *
 * So the mapping is checked as a mapping. For every corpus row, every numeric
 * literal in the shCAD source is bumped by one, ON ITS OWN, and the converted
 * text has to CHANGE. A slot that is a function of its argument moves; a slot
 * that has been spelled out does not, and says which literal it stopped
 * tracking. It costs no new corpus rows, no new geometry and no second source
 * of truth, and a thirteenth shCAD name inherits it the day its rule lands.
 *
 * Two honest limits, so nobody reads more into a green than is there:
 *   * a digit inside a string or a comment passes trivially, because the
 *     converter copies both verbatim. That is a weak sample, not a false one —
 *     the positional slots each have their own literal in the same text, and
 *     those are the ones that fail.
 *   * it proves the emitted text MOVED with the argument, not that it moved
 *     correctly. Correctness is what the row's own text-and-geometry checks are
 *     for; this is the check that they are being asked a question.
 */
export const PORTABLE_RESPONDS = {
  /** how a literal is disturbed: enough to change the text, never enough to change the grammar */
  bump: (literal) => String(Number(literal) + 1),
  /**
   * Rows whose shCAD text carries no number at all would be vacuous. There are
   * none today, and a row that becomes one has to be listed here with a reason
...
/**
 * Numeric literals in a fragment of source, left to right.
 *
 * The lookbehind keeps it off the `2` in `geom2` and off a property like
 * `v.0` — a bare number in argument position is what this is for. Deliberately
 * self-contained rather than reusing the converter's own scanner: a gate that
 * borrows the thing it is measuring measures nothing.
 */
...
/** Replace the literal at `slot` with `value`, leaving every other byte alone. */
export function replaceLiteral(text, slot, value) {
