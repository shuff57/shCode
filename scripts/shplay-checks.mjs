// shplay-checks.mjs — named behaviour checks for the shPlay gate.
//
...
export const SEMANTIC_CHECKS = [
  ...
  // count (correct, matches q5play) silently broke the movement idiom three
...

  {
    ...
    run({ runSketch }) {
      ...
      if (bad.length) {
        return bad.join(', ') + " declare no callback parameter. q5play gives all six " +
  ...
  },
  {
    ...
    run({ createSandbox }) {
      ...
      if (!(p.leftFromA > 0)) return "pressing 'a' did not also register as 'left' (q5play's _simpleKeyControls)";
  ...
  },
  {
    ...
    // still down must not stop the player. q5play releases the shared alias
  ...
  },
  {
    ...
    run({ createSandbox }) {
      ...
      if (moved.world !== 620) return 'mouse.x should follow the camera (620), got ' + moved.world;
      if (moved.screen !== 120)
  ...
  {
    name: 'kb.space exposes the raw signed counter, matching q5play',
    area: 'input',
    informational: true,
    // q5play's named-key properties ARE the raw counter, so on the release
    ...
    run({ createSandbox }) {
      ...
      return 'kb.space read ' + v + ' on the release frame; q5play would give -1 or -2 ' +
        '(truthy). shPlay returns kb.pressing()\'s value so `if (kb.space)` cannot ' +
  ...
  },
  {
    ...
    // never implemented in q5play itself — ours is written to the documented
    ...
    run({ runSketch }) {
      ...
      if (b.zero.x === 0 || b.zero.y === 0)
        return 'scale = 0 left ' + JSON.stringify(b.zero) + '; ctx.scale(0,0) is a degenerate matrix and ' +
          'a browser draws nothing after it. q5play clamps to 0.01 (q5play.js:2055).';
      ...
      // the array form q5play's own typings advertise
      ...
      if (b.arrZero.x === 0 || b.arrZero.y === 0)
        return 'scale = [0,0] left ' + JSON.stringify(b.arrZero) + '; the array form needs the same zero-clamp. ' +
          "Real q5play misses this — its object/array branch uses ?? , which does not catch literal 0.";
  ...
  },
  {
    ...
    // the range where shortest-path and q5play's raw delta AGREE -- so it
    // passed while a real divergence sat underneath it. These are the angles
    // that disagree. Deliberate divergence, D17: q5play normalises only its
      ...
      for (const [target, expected] of Object.entries(want)) {
        ...
        if (Math.abs(got - expected) > 1e-6) {
          return 'rotateTowards(' + target + ', 0.1) from rotation 0 set angularVelocity ' + got +
            ...
            Math.round(Number(target) * 0.1 * 100) / 100 + ' — that is q5play\'s plain-number ' +
