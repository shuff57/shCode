...
- The frames cycle automatically — q5play advances through them every game frame and loops.
...
function setup() {
  ...
  // q5play also sets it as the active animation automatically.
  splat.addAni('explode', '/q5play/assets/asterisk_explode.avif', 11);
}
...
**Sprite-sheet anatomy.** The image at `/q5play/assets/asterisk_explode.avif` is one wide PNG-style file containing 11 frames in a row. q5play divides the sheet's width by `frameCount` to compute each frame's slice size. If you have multiple animations packed into a multi-row sheet, use `addAnis` (plural) with an atlas object — that's covered in `2.4.5 Animated Sprites Sandbox`.
...
| **Frame (animation)** | One slice of the sprite sheet; q5play advances through them automatically. |
