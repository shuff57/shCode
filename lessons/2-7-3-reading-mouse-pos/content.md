...
**Read before `2.7.4 Reading — q5play docs: mouse.pressing()`.** About 5 minutes.

`mouse.x` and `mouse.y` are numbers that q5play updates every frame. They hold the cursor's position in canvas coordinates — the same coordinate space your sprites live in. Top-left of the canvas is `(0, 0)`. Bottom-right is `(width, height)`.

Because q5play updates them before each call to `draw()`, you can use them anywhere inside `draw()` without any extra setup.
