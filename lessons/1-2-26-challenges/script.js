// 1.2.26 Challenges — Optional Stretch

// CHALLENGE 1: FizzBuzz
//   Loop through every integer from 1 to 20.
//   For each number:
//     - If it is divisible by BOTH 3 and 5, log "FizzBuzz".
//     - If it is divisible by only 3, log "Fizz".
//     - If it is divisible by only 5, log "Buzz".
//     - Otherwise log the number itself.
//   Hint: check the most specific case (divisible by both) first.

// CHALLENGE 2: Count vowels
//   Start with any string of your choice.
//   Write code that counts how many characters in the string are vowels
//   (a, e, i, o, u — count upper and lower case).
//   Log the total vowel count.

// CHALLENGE 3: Sum even numbers
//   Loop through every integer from 1 to 20.
//   Add up only the even numbers (use % to test) and log the total.
