**Goal:** Combine hit-testing (6.7.6), one-shot click (6.7.5), held-button detection (6.7.4), and position-override with velocity zeroing (6.7.9) to drag a sprite with the mouse.
...
**Start the drag**: `mouse.presses()` fires only on the first frame the button goes down (one-shot, from 6.7.5). The `&&` check calls `world.getSpriteAt(mouse.x, mouse.y)` to ask which sprite is under the cursor (from 6.7.6). The drag only starts if that sprite is exactly `s`. Clicking empty canvas does nothing.
...
**Move the sprite**, while dragging, `s.pos.x` and `s.pos.y` are forced to the cursor position. `s.vel.x = 0; s.vel.y = 0` erases any velocity the physics engine might have assigned (from 6.7.9). Without the velocity zero, the physics engine fights the position override and the sprite shakes or drifts.
