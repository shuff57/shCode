...
A single static `star` sprite using `sprite.image` with the moSHion `star.webp` asset. There's no `addAni`, no frame cycle, just one image rendered every frame.
...
function setup() {
  ...
  star.image = '/moshion/assets/star.webp';
}
