# shplay Challenge Lesson Conventions

A **challenge is a harder shplay assignment** — same lesson-type shape as a lab (`lab-assignment-conventions.md`), but with a **permissive grader** (alternation / "any of these techniques passes") and `type === "challenge"` so the sidebar shows the ⭐ badge. Everything else — Steps scaffold in `script.js`, docs-pointer hints, UI behavior, title rule — comes from `shplay-lesson-conventions.md`. When a module spec lists a "challenges" entry or a `lessons/<slug>/` has `lesson.json.type === "challenge"`, these rules are binding.

**Applies to:**
- `lessons/<slug>/lesson.json` where `type === "challenge"` (and typically `preview === "shplay"`).

**Canonical example:** `lessons/2-1-11-challenges/` (2.1.12 Challenges — Optional Stretch).

Read `shplay-lesson-conventions.md` and `lab-assignment-conventions.md` first — this doc only covers the three things that make a challenge different from a lab.
...
```json
{
  ...
  "slos": ["SLO-3"],
  "steps": [
    { "id": "s1", "title": "Create a canvas", "instructions": "Inside setup(), create a canvas of any size.", "hints": ["Open the shplay docs drawer on the right and find the Canvas section."] },
    { "id": "s2", "title": "Create at least one sprite", "instructions": "In setup(), create a sprite …", "hints": ["Check the Sprite section of the shplay docs drawer for constructor arguments."] },
    { "id": "s3", "title": "Clear the background each frame", "instructions": "Call background() …", "hints": ["Search the shplay docs drawer for background()."] },
...
- **Do not give specific hints in `lesson.json.steps[].hints[]`.** Generic docs-pointer only (see `shplay-lesson-conventions.md` §2). Challenge menu in `content.md` may be more generous.
...
| Workspace consolidation alignment | §7 trimmed to a thin pointer at the new `shplay-lesson-conventions.md` §5. Removed the stale recap of "auto-open docs drawer / left sidebar Grading tab / drag-resizable sidebars" — the workspace is now the single right-edge `TabbedRightDrawer` with Docs / Quest / File tabs (closed by default) plus the bottom `LessonProgressFooter`. |
| Difficulty tiers slot-type | Challenges `difficulty` field bumped from `"intermediate"` to `"advanced"` to encode the lesson/lab/challenge progression: practice = `"beginner"`, lab = `"intermediate"`, challenge = `"advanced"`. §1 JSON shape + the "fields that differ from a lab" comparison table updated to a three-column lesson/lab/challenge view. Sister conventions `shplay-lesson-conventions.md` and `lab-assignment-conventions.md` carry the matching rule. Audit pass on existing unit-2 challenges (2.1.12, 2.2.13, 2.3.12) bumped `"intermediate"` → `"advanced"`. |
