...
Distinct from a moSHion lab (`preview: "moSHion"`) because there is no `script.js` and no regex grading: the grade comes from an LLM evaluating the student's prose against a rubric. See `components/ContentLessonView.tsx` line 134 for the branch that routes an `aiGrader`-bearing lesson to `WrittenGrader`.
...
(`lessons/1-1-23-a3-3-unit-quiz/` is displayed as *1.1.23*). Never cite a lesson that comes
...
- **Do not use `preview: "moSHion"` for a writeup**: the UI won't mount `WrittenGrader` and there's no script to grade.
