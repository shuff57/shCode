...
export function indexesCorners(c) {
...
export function cornersOf(c) {
    if (c.kind === 'lock')
        return [c.corner];
    if (c.kind === 'symmetric')
        return [c.a, c.b, c.center];
    return [c.a, c.b];
}
/** Edge n runs from corner n to corner n+1, wrapping at the end. */
export function edgeCorners(n, count) {
    return [((n % count) + count) % count, ((n + 1) % count + count) % count];
}
export function edgeLength(pts, n) {
 ...
 */
function edgeAngle(pts, n, count) {
 ...
 */
export function solveSketch(input, constraints, pinned = []) {
    const pts = input.map((p) => [p[0], p[1]]);
    const n = pts.length;
    ...
    const rules = (x) => residualsOf(expand(x), constraints, startScale);
    ...
    const runPass = (start) => {
        const guided = leastSquares(start, (x) => {
            const r = rules(x);
            for (let j = 0; j < x.length; j++)
                r.push(HOME_PULL * (x[j] - start[j]));
            return r;
    ...
    const hasZeroLengthEdge = Array.from({ length: n }, (_, i) => edgeLength(pts, i)).some((len) => len < 1e-9);
...
export function residualsOf(pts, constraints, scaleOverride) {
    const n = pts.length;
    ...
    return constraints.map((c) => {
        ...
        if (c.kind === 'length')
            return Math.abs(edgeLength(pts, c.edge) - c.value);
        ...
        if (c.kind === 'distanceX')
            return Math.abs(pts[c.b][0] - pts[c.a][0] - c.value);
        if (c.kind === 'distanceY')
            return Math.abs(pts[c.b][1] - pts[c.a][1] - c.value);
        if (c.kind === 'symmetric') {
...
function residualOfPair(pts, c, n, scale) {
    if (c.kind === 'equal') {
...
export function residualOf(pts, constraints) {
    ...
    let worst = 0;
    for (const r of residualsOf(pts, constraints))
 ...
 * to nothing.
 */
export function collapsedByRatio(before, after) {
    const n = before.length;
    if (n !== after.length || n < 3)
        return false;
    for (let i = 0; i < n; i++) {
        const b = edgeLength(before, i);
        if (b < 1e-9)
            continue; // already had no length to lose -- not this gate's job
        if (edgeLength(after, i) < b * COLLAPSE_RATIO)
            return true;
    }
 ...
 */
function fewestMoversSeed(pts, edge, other, kind, count) {
    const seed = pts.map((p) => [p[0], p[1]]);
    const angleA = edgeAngle(pts, edge, count);
    if (angleA === null)
        return seed; // nothing to align to -- fall back to the plain solve
    const [a1, a2] = edgeCorners(edge, count);
    const [b1, b2] = edgeCorners(other, count);
    const shared = [b1, b2].find((c) => c === a1 || c === a2);
    const pivot = shared !== undefined
        ? pts[shared]
    ...
    if (kind === 'equal') {
        ...
        for (const c of [b1, b2]) {
            if (c === shared)
                continue; // the pivot corner does not move by definition
            seed[c] = [
 ...
 */
export function seedForNewRule(points, constraints) {
...
export function describe(c) {
    if (c.kind === 'horizontal')
        return `edge ${c.edge + 1} across`;
    if (c.kind === 'vertical')
        return `edge ${c.edge + 1} up`;
    if (c.kind === 'length')
        return `edge ${c.edge + 1} = ${c.value}`;
    if (c.kind === 'equal')
        return `edge ${c.edge + 1} = edge ${c.other + 1}`;
    if (c.kind === 'parallel')
        return `edge ${c.edge + 1} ∥ edge ${c.other + 1}`;
    if (c.kind === 'perpendicular')
        return `edge ${c.edge + 1} ⊥ edge ${c.other + 1}`;
    if (c.kind === 'distanceX')
        return `corner ${c.a + 1}→${c.b + 1} across = ${c.value}`;
    if (c.kind === 'distanceY')
        return `corner ${c.a + 1}→${c.b + 1} up = ${c.value}`;
    if (c.kind === 'symmetric')
        return `corner ${c.center + 1} centred between ${c.a + 1} and ${c.b + 1}`;
    if (c.kind === 'angle')
        return `edge ${c.edge + 1} ∠ edge ${c.other + 1} = ${c.degrees}°`;
...
function describeQuality(c) {
    if (c.kind === 'horizontal')
        return 'stay level';
    if (c.kind === 'vertical')
        return 'stay upright';
    if (c.kind === 'length')
        return `stay ${c.value} long`;
    if (c.kind === 'equal')
        return `stay the same length as edge ${c.other + 1}`;
    if (c.kind === 'parallel')
        return `stay parallel to edge ${c.other + 1}`;
    if (c.kind === 'perpendicular')
        return `stay at a right angle to edge ${c.other + 1}`;
    if (c.kind === 'distanceX')
        return `stay ${c.value} across from corner ${c.a + 1}`;
    if (c.kind === 'distanceY')
        return `stay ${c.value} up from corner ${c.a + 1}`;
    if (c.kind === 'symmetric')
        return `stay centred between corners ${c.a + 1} and ${c.b + 1}`;
    if (c.kind === 'angle')
        return `stay at ${c.degrees}° to edge ${c.other + 1}`;
    return 'stay pinned in place';
}
function subjectOf(c, lower) {
    if (c.kind === 'lock')
        return (lower ? 'c' : 'C') + `orner ${c.corner + 1}`;
    ...
    if (c.kind === 'distanceX' || c.kind === 'distanceY')
        return (lower ? 'c' : 'C') + `orner ${c.b + 1}`;
    if (c.kind === 'symmetric')
        return (lower ? 'c' : 'C') + `orner ${c.center + 1}`;
...
export function describeRemovalNote(removed, added) {
...
function describePairGoal(c) {
    if (c.kind === 'equal')
        return 'match in length';
    if (c.kind === 'parallel')
        return 'run parallel';
    if (c.kind === 'angle')
        return `meet at ${c.degrees}°`;
...
export function describeRemovalNotePair(removed, removedAlso, added) {
 ...
 */
export function addConstraintSettling(points, next) {
    if (next.length === 0)
        return { constraints: next, removed: null, removedAlso: null };
...
export function losingEdges(pts, constraints) {
    const n = pts.length;
    const wrap = (e) => ((e % n) + n) % n;
