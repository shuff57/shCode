...
import { arcFromBulge, bowOf, segmentRoles } from './sketch-arc.js';
import { edgeCorners, edgeLength } from './sketch-solve.js';
 ...
 * through unchanged.
 */
export function treatmentsFromOutline(design, points, basis, bulges) {
    const rounds = {};
    const chamfers = {};
    const edgeBulges = {};
    const corners = [];
 ...
 */
export function circleLabel(points) {
    if (points.length !== 2)
        return null;
    const [a, b] = points;
 ...
 */
export function sketchLabels(points, constraints = [], rounds, chamfers, bulges) {
    const count = points.length;
    if (count < 2)
        return { edges: [], corners: [], bows: [] };
    const centroid = centroidOf(points);
    const lengthRuled = new Set();
    for (const c of constraints)
        if (c.kind === 'length')
            lengthRuled.add(c.edge);
    const edges = [];
    for (let e = 0; e < count; e++) {
        if (bulges?.[e])
            continue; // curved -- see the bows loop below instead
        ...
        if (len < 1e-9)
            continue; // nothing to measure on a collapsed edge
    ...
    for (const [key, bulge] of Object.entries(bulges ?? {})) {
        const e = Number(key);
        if (!Number.isInteger(e) || e < 0 || e >= count || !bulge)
            continue;
        const [a, b] = edgeCorners(e, count);
        const pa = points[a];
        const pb = points[b];
 ...
 */
function slideClear(cur, blockers) {
    ...
    for (const b of blockers) {
        ...
        if (Math.abs(bPerp - curPerp) >= bHalfPerp + curHalfPerp)
            continue;
    ...
    if (!merged.some(([lo, hi]) => curAlong > lo && curAlong < hi))
        return false;
    let bestPoint = 0;
    let bestDir = 1;
    let bestDist = Infinity;
    for (const [lo, hi] of merged) {
        for (const [point, dir] of [[lo, -1], [hi, 1]]) {
 ...
 */
export function layoutLabels(boxes, viewport, obstacles = []) {
    const placed = boxes.map((b) => ({ ...b }));
    const STEP_ROUNDS = 8;
...
//# sourceMappingURL=sketch-outline.js.map