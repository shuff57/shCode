...
const sumSquares = (r) => r.reduce((a, v) => a + v * v, 0);
const worstOf = (r) => r.reduce((a, v) => Math.max(a, Math.abs(v)), 0);
/**
 ...
 */
export function leastSquares(start, residuals, options = {}) {
    const tolerance = options.tolerance ?? 1e-6;
    const maxIterations = options.maxIterations ?? 100;
    const epsilon = options.epsilon ?? 1e-6;
    const primary = options.primaryCount;
    const judged = (r) => (primary === undefined ? r : r.slice(0, primary));
...
//# sourceMappingURL=least-squares.js.map