...
import { dependsOn } from './model-types.js';
/**
 ...
 */
export function orphanedBy(doc, ids) {
 ...
 */
export function whyDeletingCosts(doc, ids, label) {
...
export function danglingRefs(doc) {
    ...
    for (const f of doc.features) {
        for (const t of dependsOn(f)) {
            if (!have.has(t))
                out.push({ feature: f.id, missing: t });
        }
    }
    return out;
}
 ...
 */
export function withoutFeatures(doc, ids) {
    const doomed = orphanedBy(doc, ids);
    const features = doc.features.filter((f) => !doomed.has(f.id));
    return { ...doc, features };
}
//# sourceMappingURL=model-deps.js.map