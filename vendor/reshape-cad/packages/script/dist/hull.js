...
export function convexHull(pts) {
    ...
    if (i1 < 0)
        return null; // every point in one place
    let i2 = -1;
    const line = sub(pts[i1], pts[i0]);
    for (let i = 0; i < n; i++) {
        if (i === i0 || i === i1)
            continue;
        if (Math.hypot(...cross(line, sub(pts[i], pts[i0]))) > eps * span) {
            i2 = i;
            break;
        }
    }
    if (i2 < 0)
        return null; // every point on one line
    let i3 = -1;
    const base = [i0, i1, i2];
    for (let i = 0; i < n; i++) {
        if (i === i0 || i === i1 || i === i2)
            continue;
        if (Math.abs(above(pts, base, pts[i])) > eps * span * span) {
            i3 = i;
            break;
        }
    }
    if (i3 < 0)
        return null; // every point on one plane
    ...
    const sees = (f, p) => f.nx * p[0] + f.ny * p[1] + f.nz * p[2] - f.d > f.tol;
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (visible.length === 0)
            continue; // already inside the hull
        ...
        for (const f of visible) {
            const [a, b, c] = f.t;
            if (!dir.has(b * n + a))
                faces.push(makeFace(a, b, i));
            if (!dir.has(c * n + b))
                faces.push(makeFace(b, c, i));
            if (!dir.has(a * n + c))
                faces.push(makeFace(c, a, i));
        }
    }
    const used = new Set();
    for (const f of faces) {
 ...
 */
export function whyNoHull(pts) {
    if (pts.length < 4) {
        return `A hull needs at least four corners to have any thickness, and this has ${pts.length}.`;
    }
    if (convexHull(pts) !== null)
        return null;
    const same = (i) => pts.every((p) => Math.abs(p[i] - pts[0][i]) < 1e-9);
 ...
 */
export function hullVolume(pts, h) {
    let v = 0;
...
export function convexHull2(pts) {
    ...
    if (!(span > 0))
        return null; // every point in the same place
    const tol = span * (span * 1e-9); // eps * span, eps = span * 1e-9
    const order = [...pts.keys()].sort((i, j) => (pts[i][0] !== pts[j][0] ? pts[i][0] - pts[j][0] : pts[i][1] - pts[j][1]));
 ...
 */
export function hull2Area(pts, h) {
    let a = 0;
    const b = h.boundary;
