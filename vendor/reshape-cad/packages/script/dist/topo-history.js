 ...
 * invalidates it.
 */
export function listShapes(oc, list) {
    if (!list)
        return [];
 ...
 */
export function faceFate(oc, op, face) {
    if (!op || !face)
        return { kind: 'deleted' };
    let gone = false;
    try {
    ...
    if (gone) {
        ...
        if (gen.length === 0)
            return { kind: 'deleted' };
        if (gen.length === 1)
            return { kind: 'replaced', face: gen[0] };
        return { kind: 'split', pieces: gen };
    }
    const mod = listShapes(oc, op.Modified(face));
    if (mod.length === 0)
        return { kind: 'kept', face };
    if (mod.length === 1)
        return { kind: 'replaced', face: mod[0] };
 ...
 */
export function uvOnFace(oc, face, pnt) {
...
export function fractionOnFace(oc, face, pnt) {
...
export function pointAtFraction(oc, face, u, v) {
    const r = uvRange(oc, face);
 ...
 */
export function distanceTo(oc, pnt, shape) {
...
export function edgesOf(oc, shape) {
    const out = [];
    const exp = new oc.TopExp_Explorer(shape, oc.TopAbs_ShapeEnum.TopAbs_EDGE, oc.TopAbs_ShapeEnum.TopAbs_SHAPE);
    while (exp.More()) {
...
export function edgeThrough(oc, shape, pnt) {
 ...
 */
export function generatedFrom(oc, op, edge) {
    if (!op || !edge)
        return null;
    let made = [];
    try {
 ...
 */
export function capOf(oc, op, end, closed) {
    if (!op || closed)
        return null;
    try {
 ...
 */
export function sharedEdge(oc, a, b) {
    if (!a || !b)
        return null;
...
export function filleted(oc, shape, edge, radius) {
    if (!shape || !edge || !(radius > 0))
        return null;
    return refusable(() => {
...
export function chamfered(oc, shape, edge, distance) {
    if (!shape || !edge || !(distance > 0))
        return null;
    return refusable(() => {
 ...
 */
export function drafted(oc, shape, face, pull, angleRad, neutralZ) {
    if (!shape || !face)
        return null;
    return refusable(() => {
        const dr = new oc.BRepOffsetAPI_DraftAngle(shape);
        const plane = new oc.gp_Pln(new oc.gp_Pnt(0, 0, neutralZ), new oc.gp_Dir(pull[0], pull[1], pull[2]));
        dr.Add(oc.TopoDS.Face(face), new oc.gp_Dir(pull[0], pull[1], pull[2]), angleRad, plane);
 ...
 */
export function placed(oc, shape, trsf) {
    if (!shape)
        return null;
    if (!trsf)
        return shape;
 ...
 */
export function pointOnFace(oc, face) {
    const c = centreOf(oc, face);
    if (distanceTo(oc, c, face) <= ON_TOL)
        return c;
    const ad = new oc.BRepAdaptor_Surface(oc.TopoDS.Face(face), true);
    ...
    for (let i = 1; i <= N; i++) {
        for (let j = 1; j <= N; j++) {
            ...
            if (distanceTo(oc, p, face) <= ON_TOL)
                return p;
        }
    }
    return null;
 ...
 */
export function pieceContaining(oc, pieces, pnt) {
