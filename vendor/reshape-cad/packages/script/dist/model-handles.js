...
import { isShape, sketchBBoxCentre } from './model-types.js';
...
function moveHandles(id, centre, reach) {
    return AXES.map(({ n, v }) => ({
        kind: 'move',
        param: `${id}_${n}`,
...
function turnHandles(id, centre, reach) {
 ...
 * anchor from.
 */
export function planeAnchor(plane, offset) {
...
function sketchHandles(f) {
    const { u, v } = planeAxes(f.plane);
    const n = f.plane === 'xy' ? [0, 0, 1] : f.plane === 'xz' ? [0, 1, 0] : [1, 0, 0];
    const world = (pu, pv) => [
    ...
    const corners = f.points.map(([pu, pv], i) => ({
        kind: 'point',
        param: `${f.id}_p${i}u`,
        paramV: `${f.id}_p${i}v`,
        origin: world(pu, pv),
    ...
    for (const [key, want] of Object.entries(f.rounds ?? {})) {
        const k = Number(key);
        if (!Number.isInteger(k) || k < 0 || k >= count || !(want > 0))
            continue;
        ...
        if (interior < RADIUS_HANDLE_MIN || interior > RADIUS_HANDLE_MAX)
            continue;
...
function pocketHandles(f, doc) {
    ...
    if (!doc.features.some((x) => x.id === f.into))
        return [];
    const plane = sk.plane ?? 'xy';
    const { u, v } = planeAxes(plane);
    const n = planeNormal(plane);
...
export function handlesFor(f, doc) {
    ...
    if (f.kind === 'box') {
        ...
        size.push({ kind: 'size', param: `${f.id}_width`, origin: [cx + w / 2, cy, cz], axis: [1, 0, 0], scale: 2, label: 'width' }, { kind: 'size', param: `${f.id}_depth`, origin: [cx, cy + d / 2, cz], axis: [0, 1, 0], scale: 2, label: 'depth' }, { kind: 'size', param: `${f.id}_height`, origin: [cx, cy, cz + h / 2], axis: [0, 0, 1], scale: 2, label: 'height' });
    }
        ...
        { kind: 'size', param: `${f.id}_radius`, origin: [cx + f.radius, cy, cz], axis: [1, 0, 0], scale: 2, label: 'across' }, { kind: 'size', param: `${f.id}_height`, origin: [cx, cy, cz + f.height / 2], axis: [0, 0, 1], scale: 2, label: 'height' });
    }
    else if (f.kind === 'cone') {
        reach = Math.max(f.radius * 2, f.height) * 0.75;
        size.push({ kind: 'size', param: `${f.id}_radius`, origin: [cx + f.radius, cy, cz - f.height / 2], axis: [1, 0, 0], scale: 2, label: 'across' }, { kind: 'size', param: `${f.id}_height`, origin: [cx, cy, cz + f.height / 2], axis: [0, 0, 1], scale: 2, label: 'height' });
    }
    else if (f.kind === 'torus') {
        reach = (f.ringRadius + f.tubeRadius) * 1.4;
        size.push({ kind: 'size', param: `${f.id}_ring`, origin: [cx + f.ringRadius, cy, cz], axis: [1, 0, 0], scale: 2, label: 'across' }, { kind: 'size', param: `${f.id}_tube`, origin: [cx + f.ringRadius, cy, cz + f.tubeRadius], axis: [0, 0, 1], scale: 2, label: 'tube across' });
    }
    else {
        reach = f.radius * 1.5;
        size.push({ kind: 'size', param: `${f.id}_radius`, origin: [cx + f.radius, cy, cz], axis: [1, 0, 0], scale: 2, label: 'across' });
    }
 ...
 * nothing.
 */
export function featureCenter(f, doc) {
    if (isShape(f))
        return f.center;
    if (f.kind === 'sketch') {
        const { u, v } = planeAxes(f.plane);
    ...
    if (f.kind === 'extrude')
        return extrudeHandles(f, doc)[0]?.origin ?? null;
    if (f.kind === 'pocket')
        return pocketHandles(f, doc)[0]?.origin ?? null;
    if (f.kind === 'fillet')
        return filletHandles(f, doc)[0]?.origin ?? null;
    return null;
}
//# sourceMappingURL=model-handles.js.map