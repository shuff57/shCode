...
import { newShape, newSketch, RECTANGLE_CONSTRAINTS, extentAlong, } from './model-types.js';
...
function hasRectangleConstraints(constraints) {
    if (!constraints || constraints.length !== RECTANGLE_CONSTRAINTS.length)
        return false;
    return RECTANGLE_CONSTRAINTS.every((want, i) => {
        const got = constraints[i];
        return got?.kind === want.kind && 'edge' in got && got.edge === want.edge;
    });
}
/**
 ...
 */
function rectDims(points) {
    if (points.length !== 4)
        return null;
    const [p0, p1, p2, p3] = points;
    if (!near(p0[1], p1[1]) || !near(p1[0], p2[0]) || !near(p2[1], p3[1]) || !near(p3[0], p0[0]))
        return null;
...
function faceWord(part) {
    return FACE_WORD[part] ?? part;
}
const MIRROR_WORD = { yz: 'left-right', xz: 'front-back', xy: 'top-bottom' };
const PLANE_WORD = { xy: 'top', xz: 'front', yz: 'side' };
...
function chainVars(doc) {
    const varOf = new Map();
    for (const f of doc.features) {
        const inherits = f.kind === 'hole' || f.kind === 'shell' || f.kind === 'fillet' || f.kind === 'draft' ||
...
function defaultCenter(doc, upTo, kind) {
    ...
    return newShape({ version: 1, features: doc.features.slice(0, upTo) }, kind).center;
}
function emitAt(bindings, featureId, center, def) {
    const bound = ['x', 'y', 'z'].some((s) => bindings.has(pname(featureId, s)));
    if (!bound && center.every((v, i) => near(v, def[i])))
        return '';
...
function draftFromWord(doc, rootId, pull, neutral) {
    const AXIS_WORDS = {
    ...
    if (half == null)
        return null;
    const c = f.center[pull === 'x' ? 0 : pull === 'y' ? 1 : 2];
    if (near(neutral, c - half / 2))
        return loWord;
    if (near(neutral, c + half / 2))
        return hiWord;
    return null;
...
function geomRowText(bindings, featureId, g) {
    const c = (v) => (v ? ', construction: true' : '');
    if (g.k === 'point')
        return `{ k:'point', id:${g.id}, p:[${lit9(g.p[0])}, ${lit9(g.p[1])}]${c(g.construction === true)} }`;
    if (g.k === 'line') {
...
function ruleRowText(bindings, featureId, index, r) {
    const n = (x) => String(x);
...
function optText(bindings, featureId, slot, key, literalText) {
...
export function toScript(doc, namedParams) {
    ...
    if (namedParams) {
        for (const p of namedParams) {
            ...
            if (!near(p.max, Math.max(100, Math.ceil(Math.abs(p.value) * 4) || 100)))
                opts.push(`max: ${lit(p.max)}`);
            if (p.step !== 1)
                opts.push(`step: ${lit(p.step)}`);
            if (p.caption !== p.name)
    ...
    doc.features.forEach((f, i) => {
        if (f.kind === 'box') {
            ...
            if (f.round)
                lines.push(`round(${f.id}, ${numText(bindings, f.id, 'round', lit(f.round))})`);
        ...
        if (f.kind === 'sketch') {
            ...
            if (f.geoms) {
                lines.push(`${f.id}.geom([${f.geoms.map((g) => geomRowText(bindings, f.id, g)).join(', ')}])`);
                const rules = (f.rules ?? []);
                if (rules.length > 0) {
            ...
            const isPristineDefault = !f.shape && !f.rounds && !f.chamfers && !f.bulges && rectRules &&
            ...
            if (isPristineDefault) {
                ...
                if (rect && rectRules) {
                    ...
                    const at = near(cx, 0) && near(cy, 0) ? '' : `, { at: [${lit(cx)}, ${lit(cy)}] }`;
                    lines.push(`${f.id}.rect(${numText(bindings, f.id, 'width', lit(w))}, ${numText(bindings, f.id, 'height', lit(h))}${at})`);
                }
                else {
        ...
        if (f.kind === 'hole') {
            ...
            const opts = [optText(bindings, f.id, 'diameter', 'across', lit(f.diameter))];
        ...
        if (f.kind === 'shell') {
            const opts = [optText(bindings, f.id, 'thickness', 'wall', lit(f.thickness))];
            if (f.open && f.open.cause === 'primitive')
                opts.push(`open: '${faceWord(f.open.part)}'`);
        ...
        if (f.kind === 'fillet') {
            const edge = f.edge;
            const [a, b] = edge.of;
            const wordOf = (n) => (n.cause === 'primitive' ? faceWord(n.part) : 'top');
        ...
        if (f.kind === 'draft') {
            const opts = [];
            if (f.whole)
                opts.push('whole: true');
            const from = draftFromWord(doc, f.target, f.pull, f.neutral);
            if (from)
                opts.push(`from: '${from}'`);
            else
                opts.push(`neutral: ${lit(f.neutral)}`);
            const ref = !f.whole && f.face && f.face.cause === 'primitive'
