...
// than guessed at.
import { capOf, edgesOf, faceFate, fractionOnFace, generatedFrom, pieceContaining, placed, pointAtFraction, pointOnFace, sharedEdge, } from '@shuff57/reshape-script/topo-history';
/** Every face of a shape, in the kernel's own order -- which is exactly the
 *  order nothing here is allowed to depend on. */
export function facesOf(oc, shape) {
    const out = [];
    const exp = new oc.TopExp_Explorer(shape, oc.TopAbs_ShapeEnum.TopAbs_FACE, oc.TopAbs_ShapeEnum.TopAbs_SHAPE);
    while (exp.More()) {
...
export function faceCentre(oc, face) {
 ...
 */
export function resolvePrimitiveFace(oc, shape, part) {
    ...
    if (part === 'side') {
        ...
        const same = (a, b) => a !== null && Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]) < 1e-7;
        for (const f of faces) {
            const c = faceCentre(oc, f);
            if (!same(topC, c) && !same(botC, c))
                return f;
        }
        return null;
    }
 ...
 */
export function namePrimitiveFace(oc, shape, feature, kind, face) {
 ...
 */
export function nameEdgeBetweenPrimitiveFaces(oc, shape, feature, kind, edge) {
    const adjacent = facesOf(oc, shape).filter((f) => edgesOf(oc, f).some((e) => e.IsSame(edge)));
 ...
 * in at the beginning.
 */
function chainFor(build, feature, ancestor) {
 ...
 */
function onRecordedSide(oc, pieces, hit, side) {
    ...
    const isExtreme = (cmp) => centres.every((c, i) => i === hitIndex || cmp(val, c[side.axis]));
    return side.dir === 'lo'
 ...
 */
function pushThrough(oc, chain, face, discriminator) {
    let cur = face;
    ...
    for (const rec of chain) {
        ...
        if (!hit)
            return null;
        if (discriminator.side && !onRecordedSide(oc, fate.pieces, hit, discriminator.side))
            return null;
        consumedSplit = true;
        cur = hit;
    }
    if (discriminator && !consumedSplit)
        return null;
    return cur;
}
/**
 ...
 */
function chainToFeature(build, from, to) {
    if (from === to)
        return [];
    const visited = new Set([from]);
    let frontier = [{ feature: from, chain: [] }];
    while (frontier.length) {
        ...
        for (const { feature, chain } of frontier) {
            for (const candidate of build.ops.keys()) {
                ...
                if (!list.some((r) => r.inputs.includes(feature)))
                    continue;
 ...
 */
export function resolveNameAsUsedBy(oc, name, build, usedByFeature) {
    ...
    if (!found || name.feature === usedByFeature)
        return found;
 ...
 */
export function resolveName(oc, name, build) {
 ...
 * finds out.
 */
export function nameCarried(oc, build, feature, of) {
    ...
    for (const rec of chain) {
        ...
        if (fate.kind === 'deleted' || fate.kind === 'split')
            return null;
    }
 ...
 * guessing at either).
 */
function siblingSide(oc, pieces, piece) {
    if (pieces.length < 2)
        return undefined;
    const centres = pieces.map((p) => centroid(oc, p));
    let axis = 0;
 ...
 */
export function nameSplitPiece(oc, build, feature, of, piece) {
    const parent = resolveName(oc, of, build);
    if (!parent)
        return null;
    let cur = parent;
    for (const rec of chainFor(build, feature, of.feature)) {
        ...
        if (!pieceContaining(oc, fate.pieces, on))
            return null;
 ...
 */
export function nameEdgeOnCurrentShape(oc, build, doc, pickedFeature, edge) {
    const pickedShape = build.shapes.get(pickedFeature);
    if (!pickedShape)
        return null;
    const adjacent = facesOf(oc, pickedShape).filter((f) => edgesOf(oc, f).some((e) => e.IsSame(edge)));
    if (adjacent.length !== 2)
        return null;
 ...
 */
export function nameFaceOnCurrentShape(oc, build, doc, pickedFeature, face) {
    const primitives = doc.features.filter((f) => f.kind === 'box' || f.kind === 'cylinder');
    for (const prim of primitives) {
        if (!build.shapes.get(prim.id))
            continue;
        for (const part of PRIMITIVE_FACE_PARTS[prim.kind]) {
            const candidate = { cause: 'primitive', feature: prim.id, kind: 'face', part };
            const resolved = resolveNameAsUsedBy(oc, candidate, build, pickedFeature);
            if (resolved && resolved.IsSame(face))
                return candidate;
        }
    }
    return null;
}
//# sourceMappingURL=topo-resolve.js.map