...
export function bboxLongestDimension(bbox) {
    return Math.max(bbox.max[0] - bbox.min[0], bbox.max[1] - bbox.min[1], bbox.max[2] - bbox.min[2]);
}
 ...
 * zero or `Infinity`.
 */
export function fitDistance(bbox, viewportWidth, viewportHeight, fovDegrees, fillFraction = DEFAULT_FILL_FRACTION, occludedWidth = 0) {
