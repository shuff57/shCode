...
function triangleIndices(mesh, t) {
    if (mesh.indices) {
 ...
 */
function triangleNormal(mesh, t) {
 ...
 */
export function fromPolygonSoup(polygons) {
    const positions = [];
 ...
 *  here. */
function build3mfModel(mesh) {
    const nV = vertexCount(mesh);
    const vertexLines = new Array(nV);
    for (let i = 0; i < nV; i++) {
        const [x, y, z] = vertexAt(mesh, i);
        vertexLines[i] = `<vertex x="${x}" y="${y}" z="${z}"/>`;
    }
    const nTri = triangleCount(mesh);
    const triLines = new Array(nTri);
    for (let t = 0; t < nTri; t++) {
        const [ia, ib, ic] = triangleIndices(mesh, t);
 ...
 */
export async function write3MF(mesh) {
    const zip = new JSZip();
