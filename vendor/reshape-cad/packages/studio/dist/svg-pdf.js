...
function arcToCubics(x1, y1, rx, ry, rotDeg, large, sweep, x2, y2) {
    ...
    const E = (t) => [cx + rx * cos * Math.cos(t) - ry * sin * Math.sin(t),
        cy + rx * sin * Math.cos(t) + ry * cos * Math.sin(t)];
    const Ed = (t) => [-rx * cos * Math.sin(t) - ry * sin * Math.cos(t),
...
function pathOps(d, n) {
    ...
    while (i < t.length) {
        ...
        switch (cmd) {
            ...
            default:
                throw new Error(`svgToPdf: unsupported path command '${cmd}' -- exportDrawing() emits `
...
function transformOps(tr, n) {
    ...
    while ((m = RE.exec(tr))) {
        ...
        if (f === 'translate') {
            ...
            if (v.length >= 3)
                out.push(`1 0 0 1 ${n(v[1])} ${n(v[2])} cm`);
    ...
    if (!seen && tr.trim())
        throw new Error(`svgToPdf: unparseable transform '${tr}'`);
    return out;
}
/**
...
export function svgToPdf(svg) {
    ...
    if (!root)
        throw new Error('svgToPdf: no <svg> root element found');
    ...
    function paint(st) {
        ...
        if (s) {
            ...
            ops.push(da && da !== 'none'
                ...
                : '[] 0 d');
            if (st['stroke-linecap'] in CAP)
                ops.push(`${CAP[st['stroke-linecap']]} J`);
            if (st['stroke-linejoin'] in JOIN)
                ops.push(`${JOIN[st['stroke-linejoin']]} j`);
        }
    ...
    while ((m = TAG.exec(text))) {
        ...
        if (name === 'g') {
            ops.push('q');
            if (a.transform)
                ops.push(...transformOps(a.transform, n));
            if (!selfClose)
                stack.push(st);
            else
                ops.push('Q');
    ...
    const chunks = [];
    let len = 0;
    const push = (s) => { const b = enc.encode(s); chunks.push(b); len += b.length; };
    const offsets = [];
    push('%PDF-1.4\n%\xC2\xB5\xC2\xB6\n');
    for (let i = 1; i <= N; i++) {
        offsets[i] = len;
...
//# sourceMappingURL=svg-pdf.js.map