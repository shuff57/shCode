...
export function namedPointsOf(g) {
    switch (g.k) {
        case 'point':
            return [{ at: 'a' }];
        case 'line':
...
export function pointWorld(g, at) {
    switch (g.k) {
        case 'point':
            return at === 'a' ? { x: g.p[0], y: g.p[1] } : null;
        case 'line':
            if (at === 'a')
                return { x: g.a[0], y: g.a[1] };
            if (at === 'b')
                return { x: g.b[0], y: g.b[1] };
            return null;
        case 'circle':
...
export function distToCircleStroke(p, center, r) {
...
export function angleInArcRange(theta, a0, a1) {
...
export function sampleArc(cx, cy, r, a0, sweep, steps = 24) {
    const pts = [];
...
export function lineAngleDeg(from, to) {
    const deg = (Math.atan2(to.y - from.y, to.x - from.x) * 180) / Math.PI;
    return deg < 0 ? deg + 360 : deg;
}
/** Does the candidate line want to be horizontal or vertical? */
export function inferLineConstraint(from, to, angleTolDeg = 4) {
    if (from.x === to.x && from.y === to.y)
        return null;
    const deg = lineAngleDeg(from, to);
    const near = (t) => Math.min(Math.abs(deg - t), 360 - Math.abs(deg - t)) <= angleTolDeg;
    if (near(0) || near(180))
        return 'horizontal';
    if (near(90) || near(270))
        return 'vertical';
...
export function arcFromClicks(c1, c2, c3) {
...
export function arcEnds(cx, cy, r, a0, sweep) {
    return {
...
export function nextGeomId(geoms) {
    let max = 0;
    for (const g of geoms)
        if (g.id > max)
            max = g.id;
...
export function renumber(geoms, rules, removedId) {
    const shift = (id) => (id > removedId ? id - 1 : id);
    const ends = ['a', 'b', 'c', 'aEnd', 'bEnd', 'cEnd'];
    const kept = geoms
        .filter((g) => g.id !== removedId)
        .map((g) => {
        const out = { ...g, id: shift(g.id) };
        return out;
    });
    ...
    const idFields = ['a', 'b', 'c'];
    const namesRemoved = (r) => idFields.some((f) => typeof r[f] === 'number' && r[f] === removedId);
    const keptRules = rules
        .filter((r) => !namesRemoved(r))
        .map((r) => {
        const out = { ...r };
        for (const e of ends) {
            if (typeof out[e] === 'number')
                out[e] = shift(out[e]);
        }
        return out;
    });
...
export function readSolved(geoms, params) {
    let i = 10;
...
export function toggleConstruction(geoms, ids) {
...
export function trimPick(geoms, clickedId, click) {
    const clicked = geoms.find((g) => g.id === clickedId);
    if (!clicked || clicked.k !== 'line')
        return null;
...
export function slotRows(cA, cB, rPoint, baseId) {
    ...
    const arc1a = [cA.x + px * r, cA.y + py * r];
    const arc1b = [cA.x - px * r, cA.y - py * r];
    const arc2a = [cB.x - px * r, cB.y - py * r];
    const arc2b = [cB.x + px * r, cB.y + py * r];
...
function tangentContact(c, line) {
...
export function splitWeldedCircles(geoms, rules) {
    const replacedIds = [];
    let geomsOut = [...geoms];
    let rulesOut = [...rules];
    for (const g of geoms) {
        if (g.k !== 'circle')
            continue;
        const tangents = rulesOut.filter((r) => r.k === 'tangent' && (r.a === g.id || r.b === g.id) && !r.aEnd && !r.bEnd);
        ...
        for (const x of geomsOut) {
            if (x.id === g.id) {
                geomsNext.push({ ...x, k: 'arc', a: [p1.x, p1.y], b: [p2.x, p2.y], sense: sense1, c: [c.x, c.y], r });
            }
            else {
                geomsNext.push(x);
            }
...
const ID_RULE_FIELDS = ['a', 'b', 'c'];
function dupRows(geoms, rules, ids, xf, newIdOf) {
    ...
    for (const r of rules) {
        const refs = ID_RULE_FIELDS.map((f) => r[f]).filter((v) => typeof v === 'number');
        if (refs.length === 0 || !refs.every((v) => set.has(v)))
            continue;
        const copy = { ...r };
        for (const f of ID_RULE_FIELDS) {
            if (typeof copy[f] === 'number')
                copy[f] = idMap.get(copy[f]);
        }
        rulesOut.push(copy);
    }
 ...
 *  backwards. */
export function mirrorSelection(geoms, rules, ids, axis) {
...
export function densifyIds(geoms, rules) {
