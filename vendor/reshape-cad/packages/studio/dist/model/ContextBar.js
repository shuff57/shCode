...
const FLIP_GAP = 6;
export default function ContextBar({ featureKind: _featureKind, name, anchor, chips, refusal, actions, onDismiss, canDismiss, viewWidth, }) {
    ...
    useEffect(() => {
        const onKey = (e) => {
            if (e.key === 'Escape' && (canDismissRef.current?.() ?? true))
    ...
    return (_jsxs("div", { className: "ctx-bar-host", style: { left: style.left, top: style.top }, children: [_jsxs("div", { className: 'ctx-bar' + (flip ? ' is-below' : ''), role: "toolbar", "aria-label": name + ' actions', style: { transform: flip ? 'translate(-50%, 0)' : 'translate(-50%, -100%)' }, onPointerDown: stop, onClick: stop, children: [_jsx("span", { className: "ctx-who", children: name }), _jsx("span", { className: "ctx-sep" }), refusal && _jsxs("span", { className: "ctx-refusal", children: ["\u26A0 ", refusal] }), actions.map((a) => (_jsx("button", { type: "button", className: a.primary ? 'is-primary' : undefined, title: a.title ?? a.label, onPointerDown: stop, onClick: (e) => { e.stopPropagation(); a.onRun(); }, children: a.label }, a.label))), chips.length > 0 && _jsx("span", { className: "ctx-sep" }), chips.map((c) => (_jsxs("span", { className: "ctx-chip", children: [c.label, " ", _jsx("span", { className: "ctx-chip-val", children: c.value })] }, c.label)))] }), _jsx("style", { children: `
