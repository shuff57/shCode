 ...
 */
function projectFrom(basis, basisPlane, q) {
...
function projectOutline(o, at) {
    const anchors = o.corners.map((c) => at.get(c));
    if (anchors.some((a) => !a || a.ux === undefined || a.uy === undefined))
        return null;
    const A = anchors;
    if (o.shape === 'circle' && o.points.length === 2) {
        ...
        for (let i = 0; i < samples; i++) {
            ...
            const q = [center[0] + radius * Math.cos(t), center[1] + radius * Math.sin(t)];
...
export default function HandleOverlay({ points, values, scales, onDrag, onCommit, onTap, outlines, outlineAnchors, bottomInset = 0, }) {
    ...
    function push(updates) {
        ...
        raf.current = requestAnimationFrame(() => {
            ...
            pending.current = [];
            for (const { param, value } of items)
                onDrag(param, value);
        });
    }
    ...
    const outlineRenders = (outlines ?? [])
        ...
        if (!projected || projected.pts.length < 2)
            return null;
    ...
    return (_jsxs("div", { className: "handle-layer", ref: layerRef, 
        ...
        style: bottomInset ? { bottom: bottomInset } : undefined, children: [outlineRenders.length > 0 && (_jsx("svg", { className: "sketch-lines", "aria-hidden": "true", children: outlineRenders.map(({ n, pts }) => (_jsx("polygon", { points: pts.map((p) => `${p.x},${p.y}`).join(' ') }, n))) })), points.map((a) => {
                ...
                return (_jsx("button", { type: "button", className: 'handle'
                        + (a.kind === 'move' ? ' is-move'
                            : a.kind === 'turn' ? ' is-turn'
                                : a.kind === 'radius' ? ' is-radius' : '')
                        + (on ? ' is-on' : ''), style: { left, top }, "aria-label": `Drag ${a.label}`, title: `${a.label}${typeof raw === 'number' ? ` — ${Math.round(raw * 100) / 100}` : ''}`, onPointerDown: (e) => {
                        e.preventDefault();
