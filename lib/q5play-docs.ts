...
function setup() {
  ...
  elevator = new Sprite(320, 200, 60, 10, 'kinematic');
  elevator.vel.y = 1;
}
...
.scale uniformly scales the drawn sprite (not the physics body — that stays the size you created).`,
  ...
  new Sprite(100, 100, 30);                // circle, diameter 30
  const box = new Sprite(250, 100, 40, 60); // 40×60 rectangle
  box.rotation = Math.PI / 6;               // tilted 30°

  const big = new Sprite(200, 260, 30, 30);
  big.scale = 1.5;                          // visually 1.5× larger
}
...
These are properties of each sprite. When two sprites collide, Box2D combines their values.`,
  ...
  const ball = new Sprite(200, 100, 30);
...
function update() {
  ...
  if (kb.presses('r')) player.color = 'deepskyblue';
}
...
function setup() {
  ...
  coins.diameter = 20;
  coins.collider = 'none';
  new coins.Sprite(120, 350);
  new coins.Sprite(260, 350);
}
...
function update() {
  ...
  // Ground check — physical collision
  if (kb.presses(' ') && player.colliding(ground)) {
    player.vel.y = -8;
  }
...
function setup() {
  new Canvas(400, 400);
  world.gravity.y = 10;
  // Wider-than-canvas level
  new Sprite(1000, 390, 2000, 20, 'static');
  player = new Sprite(200, 300, 40, 40);
  player.color = 'deepskyblue';
}
...
Usually one sprite in a joint is static (an anchor) and the other is dynamic.`,
  ...
  // Pendulum
  const anchor = new Sprite(130, 60, 16, 16, 'static');
  const rod = new Sprite(130, 160, 20, 120);
  new HingeJoint(anchor, rod);

  // Vertical piston
  const pistonBase = new Sprite(300, 200, 16, 16, 'static');
  const piston = new Sprite(300, 300, 60, 20);
  new SliderJoint(pistonBase, piston);
}
...
function setup() {
  ...
  chassis = new Sprite(200, 300, 80, 20);
  wheel = new Sprite(180, 320, 20);
...
function draw() {
  ...
  fill('#888');
  textSize(12);
  text('Press space to score', 14, 56);
}`,
      },
    ],
  },
