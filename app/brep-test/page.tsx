...
// TEMPORARY SPIKE PAGE. This exists to verify BrepViewport by hand -- it is
// not linked from anywhere a student reaches and should be deleted once the
// mouse-built (Build mode) UI is wired to the B-rep kernel for real. See
// components/model/BrepViewport.tsx for the component under test.

import { useState } from 'react';
import BrepViewport, { type BrepViewportStats } from '../../components/model/BrepViewport';
import {
  EMPTY_DOC,
  newExtrude,
  newRectangleSketch,
...
export default function BrepTestPage() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#282a36', color: '#f8f8f2', font: '13px ui-monospace, Menlo, Consolas, monospace' }}>
      <div style={{ width: 320, flexShrink: 0, borderRight: '1px solid #44475a', padding: 16, overflow: 'auto' }}>
        <h1 style={{ fontSize: 15, margin: '0 0 4px' }}>BrepViewport spike</h1>
        <p style={{ color: '#6272a4', margin: '0 0 16px' }}>
          Temporary verification page. Not linked from the app.
        </p>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <BrepViewport doc={doc} onStats={setStats} />
      </div>
    </div>
  );
}
